...
export default function AdminSidebar() {
  ...
  return (
    <aside className="w-72 bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 min-h-screen fixed left-0 top-0 z-40 shadow-2xl">
      {/* Logo Section */}
      <div className="p-6 border-b border-slate-700/50">
        <Link href="/admin" className="flex items-center gap-3">
          ...
          <div style={{ display: "flex", alignItems: "center" }}>
            ...
            <span className="ml-2 text-[10px] bg-gradient-to-r from-indigo-500 to-purple-500 text-white px-2 py-0.4 mt-0.5 rounded-full font-medium uppercase tracking-wider">
              Admin
            </span>
          </div>
        </Link>
        ...
        <div className="space-y-1">
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                <div
                  className={`${isActive ? "text-indigo-400" : "text-slate-400 group-hover:text-slate-300"} transition-colors`}
                >
                  {item.icon}
                </div>
                <span
                  className={`font-medium ${isActive ? "text-indigo-300" : "text-slate-300 group-hover:text-white"} transition-colors`}
                >
                  {item.name}
                </span>
                {isActive && (
